...
<p align="center">
  ...
  <a href="#เคส">เคส</a> ·
  <a href="#พาออกนอกบ้าน">พาออกนอกบ้าน</a> ·
...
ทางเดินทั้งสองทางที่ผ่านมาตั้งอยู่บนสมมติฐานเดียวกัน คือ Mac กับบอร์ดต้องหากันเจอ — Bluetooth ขอราวสิบเมตร
ส่วนทาง LAN ขอให้อยู่วงเดียวกัน ก้าวออกจากสองอย่างนี้เมื่อไหร่จอก็หยุดขยับ ถ้าอยากเอาไปวางบนโต๊ะโรงแรม
หรือในคาเฟ่ มีสองทาง แต่ระดับความมั่นใจต่างกันมาก

**Wi-Fi สาธารณะไม่ใช่หนึ่งในนั้น** เพราะหน้า login (captive portal) ต้องใช้เบราว์เซอร์ซึ่งบอร์ดไม่มี
ให้ใช้ hotspot จากมือถือแทน มันไม่มีหน้า login และอย่าลืมไฟเลี้ยง — เคสไม่มีที่ใส่แบต ต้องพก power bank

**ทางที่ 1 — Tailscale** มือถือปล่อย hotspot บอร์ดเกาะ แล้วให้มือถือรัน Tailscale เป็น subnet router
ประกาศ subnet ของ hotspot นั้น ส่วน Mac เปิด `--accept-routes` จากนั้น Mac จะเห็นบอร์ดที่ที่อยู่ใน hotspot
เหมือนอยู่วง LAN บ้านเลย ไม่ต้องรันเซิร์ฟเวอร์อะไรเอง ติดสองข้อ: **มือถือต้องเป็น Android** เพราะ iOS
เป็น subnet router ไม่ได้ และ subnet ของ hotspot ต้องไม่ชนกับวงที่บ้าน — ทางนี้ทดสอบด้วยการเดินออกไปไกลมาก
แล้วจอยังเดินอยู่

**ทางที่ 2 — relay ที่เรารันเอง** บอร์ดวิ่ง *ออก* ไปหา relay ฝั่ง Mac ก็วิ่ง *ออก* ไปที่ relay ตัวเดียวกัน
แล้วมาเจอกันตรงกลาง NAT จึงไม่เป็นปัญหาของทั้งสองฝั่ง ใช้ hotspot มือถือรุ่นไหนก็ได้ ไม่ต้องยุ่งกับ Tailscale

**relay ไม่ถือกุญแจอะไรเลย** payload ถูกปิดผนึกด้วย AES-256-GCM ตั้งแต่ก่อนออกจากปลายทั้งสอง relay จึงเห็นแต่
ข้อมูลที่ถอดไม่ได้ และไม่ต้องมี TLS ของตัวเอง — ซึ่งเป็นเหตุผลที่เรื่องนี้ทำได้จริง เพราะ TLS handshake
ขอ 20-40 KB ส่วน heap ว่างต่ำสุดที่วัดได้บนบอร์ดคือ 26.8 KB และซ็อกเก็ต TCP ขาออกกินแค่ 556 ไบต์
ตัว [`relay/perch-relay.py`](relay/perch-relay.py) ยาวราว 140 บรรทัด ไม่มี dependency
...
ไม่มีที่อยู่เริ่มต้น และค่าว่างคือปิดทางนี้ทิ้ง — relay คือเซิร์ฟเวอร์ของใครสักคน การใส่ที่อยู่ไว้เป็นค่าโรงงาน
แปลว่าบอร์ดของทุกคนที่ clone repo นี้ไปจะวิ่งไปหาเครื่องนั้นโดยเจ้าของไม่ได้ตกลงด้วย

> **ทางที่ 2 ยืนอยู่ตรงไหนจริงๆ** ฝั่งบอร์ดพิสูจน์แล้ว — มันวิ่งออกจาก hotspot มือถือบน IP มือถือได้จริง
> และค่าที่ตั้งไว้อยู่รอดข้ามการ flash เฟิร์มแวร์ที่ไม่มีธงนี้ **ส่วนฝั่ง Mac ยังไม่เคยถูกรันเลยสักครั้ง**
> เพราะทาง LAN ตรงชนะทุกครั้งที่ทดสอบ โค้ดเขียนไว้แล้วและอ่านดูเข้าท่า ซึ่งไม่ใช่เรื่องเดียวกับใช้ได้จริง
> ถ้าใครลองแล้วเป็นยังไง เขียนไว้ที่ [issue #1](https://github.com/zeuscs09/perch/issues/1) ได้เลย
