...
static pch_state_t state_from_name(const char *name)
{
    if (!name) return PCH_STATE_IDLE;
    for (int i = 0; i < PCH_STATE_COUNT; i++) {
        if (strcmp(name, pch_state_names[i]) == 0) return (pch_state_t)i;
    }
    // ชื่อที่ firmware ไม่รู้จัก = daemon ใหม่กว่า firmware ให้ยืนเฉยๆ ดีกว่าจอค้าง
    ESP_LOGW(TAG, "unknown state '%s'", name);
    return PCH_STATE_IDLE;
}
...
static pch_agent_t agent_from_name(const char *name)
{
    if (!name) return PCH_AGENT_CLAUDE;
    if (name[0] == 'x') return PCH_AGENT_CODEX;
    if (name[0] == 'g') return PCH_AGENT_ANTIGRAVITY;
    return PCH_AGENT_CLAUDE;
}

static pch_card_kind_t kind_from_name(const char *name)
{
    if (!name) return PCH_CARD_INFO;
    if (strcmp(name, "alert") == 0) return PCH_CARD_ALERT;
    if (strcmp(name, "done") == 0) return PCH_CARD_DONE;
    return PCH_CARD_INFO;
}
...
void pch_model_clear(pch_snapshot_t *s)
{
    memset(s, 0, sizeof(*s));
    strcpy(s->clock, "--:--");
    s->temperature = PCH_TEMP_UNKNOWN;
    for (int i = 0; i < PCH_USAGE_ROWS; i++) {
        s->usage[i].percent = PCH_USAGE_UNKNOWN;
        s->usage[i].remaining = PCH_USAGE_UNKNOWN;
    }
}
...
bool pch_model_parse(const char *json, int len, pch_snapshot_t *out)
{
    ...
    if (cJSON_IsArray(sessions)) {
        ...
        cJSON_ArrayForEach(it, sessions) {
            if (tmp.session_count >= PCH_SLOTS_COUNT) break;
            pch_session_t *sess = &tmp.sessions[tmp.session_count++];
    ...
    if (cJSON_IsArray(cards)) {
        ...
        cJSON_ArrayForEach(it, cards) {
            if (tmp.card_count >= PCH_MAX_CARDS) break;
            pch_card_t *card = &tmp.cards[tmp.card_count++];
    ...
    if (cJSON_IsArray(w)) {
        ...
        if (cJSON_IsNumber(cond) && cond->valueint >= 0 && cond->valueint < PCH_WEATHER_COUNT) {
            tmp.weather = (pch_weather_t)cond->valueint;
