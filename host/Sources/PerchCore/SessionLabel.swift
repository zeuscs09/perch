...
public enum SessionLabel {
    ...
    /// ไม่มี "repo" ในนี้โดยตั้งใจ — มันเป็นชื่อโปรเจกต์จริงได้ (และ upstream มีเทสต์
    /// ที่ยึดกรณีนั้นไว้) ต่างจากชื่อพวกนี้ที่ไม่มีใครตั้งเป็นชื่อโปรเจกต์
    static let containers: Set<String> = [
        "agents", "src", "app", "apps", "packages", "services", "workers",
    ]
